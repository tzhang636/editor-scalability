...
/**
 * This is a Fortran model builder which uses the Fortran parser to construct the model. See
...
public class FortranModelBuilder implements IFortranModelBuilder
{
    ...
    // one-way flag. The builder is discarded after interruption
    protected boolean interrupted = false;
    private OnParseCompleteListener parseListener;
    ...
    public void setTranslationUnit(ITranslationUnit tu)
    {
        if (!(tu instanceof TranslationUnit))
            throw new Error("Unexpected subclass of ITranslationUnit"); //$NON-NLS-1$

        this.translationUnit = (TranslationUnit)tu;
    }
    
    /**
     * Return false because the default fortran model builder isn't interruptible
     */
    ...
    public void parse(boolean quickParseMode) throws Exception
    {
        ...
        try
        {
            ...
            String filename;
            FortranElement fortranElement = null;
            
            if (file != null)
            {
                sourceForm = determineSourceForm(file);
                filename = determineFilename(file);
                fortranElement = createSourceFormNode(SourceForm.descriptionFor(file));
            }
            else
            {
                ...
                fortranElement = createSourceFormNode(SourceForm.descriptionFor(filename));
            }

            lexer = new ASTLexerFactory().createLexer(new StringReader(translationUnit.getBuffer()
                .getContents()), file, filename, sourceForm);

            // check before parsing
            // parsing is by far the longest operation (~10k+ ms upon startup)
            if (isInterrupted(fortranElement)) { return; }

            // There may be more than one FortranModelBuilder running at once, so
            // unfortunately, we have to create a new parser each time
            IFortranAST ast = new FortranAST(file, new Parser().parse(lexer), lexer.getTokenList());

            if (isParseTreeModelEnabled())
            {
                // check before replacing loops
                // ~3k+ ms upon startup
                if (isInterrupted(fortranElement)) { return; }
            ...
            // FortranElement note = new FortranElement.UnknownNode(translationUnit, isFixedForm ?
            // "<Fixed Form Source>" : "<Free Form Source>");
            // this.addF90Element(note);

            /*
             * For whatever reason, we have to only set the *parent* while we parse, then go back
             * through (now) and traverse the model in preorder, executing addChild(). This doesn't
             * make any sense, but the "obvious" way of doing it all while parsing (in
        ...
        {
            parseListener.onParseComplete(this, wasSuccessful);
        }
    ...
    public void configureElement(FortranElement elt, Token nameToken)
    {
        if (nameToken != null)
        {
            if (!(elt instanceof FortranElement.ErrorNode)
                && !(elt instanceof FortranElement.UnknownNode))
