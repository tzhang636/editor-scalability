...
public class FortranStmtPartitionScanner implements IPartitionTokenScanner
{
 ...
 /**
  * In debug mode, use pre-determined values for parameters to prevent listeners 
  * from being added for each statement (makes test cases run slowly)
  */
    ...
    public IToken nextToken()
    {
        ...
        try
        {
            final FortranLineScanner lineScanner;
            if (__debug) { 
                lineScanner = new FortranLineScanner(false, false);                
            } else {
                lineScanner = new FortranLineScanner(isFixedForm(), isCPreprocessed());
            }
