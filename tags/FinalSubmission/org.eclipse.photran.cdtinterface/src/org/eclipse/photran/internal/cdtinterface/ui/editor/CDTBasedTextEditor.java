...
public abstract class CDTBasedTextEditor extends TextEditor implements ISelectionChangedListener
{
    ...
    public Object getAdapter(Class required) {
        ...
        if (required == IShowInSource.class) {
            int lineThreshold = Integer.MAX_VALUE;
            if (FortranPreferences.SCALABILITY_ENABLE.getValue()) {
                lineThreshold = FortranPreferences.SCALABILITY_THRESHOLD.getValue();
            }
            IDocument document = this.getDocumentProvider().getDocument(getEditorInput());
            assert (document != null);
            if (document.getNumberOfLines() > lineThreshold) {
                return null;
            }
            ICElement ce= null;
            try {
